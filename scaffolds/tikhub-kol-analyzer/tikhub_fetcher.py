...
from config import TIKHUB_KEY, SEARCH_KEYWORD, VIDEOS_TO_ANALYZE
...
def _get(endpoint: str, params: dict, timeout: int = 20, max_retries: int = 3) -> dict:
    last_exc = None
...
def search_creators_by_keyword(keyword: str, max_creators: int = 50) -> list[dict]:
    """
...
def fetch_user_videos(username: str, count: int = VIDEOS_TO_ANALYZE) -> list[dict]:
    try:
...
def fetch_user_country(username: str) -> str:
    try:
...
def fetch_video_comments(video_id: str, count: int = 50) -> list[dict]:
    try:
...
def fetch_bio_url(username: str) -> str:
    ...
    try:
        data = _get("/api/v1/tiktok/app/v3/fetch_user_profile_by_unique_id", {
            "unique_id": username,
        })["data"]
        user = data.get("user", {})
...
def detect_fake_views(video_id: str) -> dict:
    try:
