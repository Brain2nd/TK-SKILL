...
# 配置会直接使用当前虚拟环境的 Python，因此 Windows/macOS/Linux 均可用。
...
运行虚拟环境中的 `python print_mcp_config.py` 会生成当前机器的绝对路径配置；Windows
使用 `.venv\\Scripts\\python.exe`，macOS/Linux 使用 `.venv/bin/python`。配置直接启动
Python MCP 服务，不依赖 Bash。等价结构如下：
...
```json
{
  "mcpServers": {
    "tiktok-kol-analyzer": {
      ...
      "args": ["/absolute/path/to/tikhub-kol-analyzer/kol_mcp_server.py"]
    }
  }
}
```
...
`get_youtube_creator`。TikTok 公开主页内容证据可使用
`collect_tiktok_public_profiles`；该工具不判定 TTS/Showcase 状态，也不会替代 FastMoss。

### 交给首次建联系统

`run_shop_discovery` 和 `read_discovery_results` 会返回 `outreach_handoff`：

- `contract_version`: 固定为 `outreach-candidate.v1`；
- `candidates_file`: 已筛选达人 CSV；
- `format`: 当前为 `csv`。

在 LOOP 中文工作台的“项目中心”点击“导入达人”，选择该文件即可。工作台会再次校验账号、联系邮箱和重复记录，并保留简介、标签与近期内容证据供 AI 个性化使用。

## 浏览器 JS 注入采集与访问保护

页面采集只执行仓库内置、可审计的提取函数，MCP 不接受任意 JavaScript 字符串。
`browser_resilience.mjs` 为 FastMoss 与 TikTok 共用以下保护：

- 仅允许访问 `https://*.fastmoss.com` 或 `https://*.tiktok.com` 白名单；
- FastMoss 默认每次页面动作间隔 2.5–6.5 秒，TikTok 默认 4–9 秒；
- 网络错误和 5xx 指数退避，429 尊重 `Retry-After`，连续限流后停止；
- 检测 401/403、CAPTCHA、滑块、人机验证及访问频繁提示，保存诊断并要求人工处理；
- 不使用 stealth 插件、指纹伪装、代理轮换、验证码绕过或 Cookie 导出；
- 每获得一条结果就写 checkpoint，使用 `--resume` 从中断处继续；
- 注入结果有大小上限，浏览器子进程不继承 MCP stdio。

FastMoss 仍需先由用户在可见浏览器中完成登录、验证码和筛选：

```bash
node fastmoss_browser.mjs --setup --setup-timeout 900
node fastmoss_browser.mjs --resume \
  --min-delay-ms 2500 --max-delay-ms 6500 \
  --output output/fastmoss_browser_candidates.csv
```

TikTok 公共主页采集示例：

```bash
node tiktok_browser.mjs \
  --username creator_one --username creator_two \
  --resume --output output/tiktok_public_profiles.csv
```

输出包含公开 Bio、邮箱、粉丝快照、近期视频证据和可取得时的均播/互动率。
`email_verified=false` 是刻意设计：页面发现邮箱后仍需由后续邮箱验证流程确认。
若页面要求登录或人机验证，命令以专用状态停止，不会自动重试绕过。
