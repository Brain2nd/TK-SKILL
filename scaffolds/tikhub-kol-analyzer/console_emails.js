...
var md5 = function(s) {
  ...
  // Fixed: extract all 4 bytes from each 32-bit integer (little-endian)
  function byteHex(v){return ((v>>>4)&0xF).toString(16)+(v&0xF).toString(16);}
  function hex(x){
    var h='';
    for(var i=0;i<x.length;i++){
      var n=x[i];
      h+=byteHex(n&0xFF)+byteHex((n>>>8)&0xFF)+byteHex((n>>>16)&0xFF)+byteHex((n>>>24)&0xFF);
    }
    return h;
  }
  return hex(md51(s));
...
function fmSign(params) {
  ...
  for (var i = 0; i < keys.length; i++) {
    raw += keys[i] + filtered[keys[i]] + SALT;
  }
  var digest = md5(raw);
  var chars = digest.split('');
  var result = [];
  var i = 0, j = chars.length - 1;
  while (i < j) {
    result.push((parseInt(chars[i], 16) ^ parseInt(chars[j], 16)).toString(16));
    i++;
    j--;
  }
  result.push(chars.slice(i).join(''));
  return result.join('');
}
...
function extractEmail(data) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (list[i].name === 'email' && list[i].has && list[i].id) {
      return list[i].id;
    }
  }
  return '';
...
(async function() {
  ...
  for (var idx = 0; idx < total; idx++) {
    ...
    var params = {
      ...
      cnonce: String(Math.floor(Math.random() * 100000000))
    };
    var sign = fmSign(params);
    var url = '/api/author/v3/detail/authorContact?uid=' + params.uid +
    ...
    try {
      ...
      results.push({ uid: uid, username: c.unique_id, error: e.message, ok: false });
