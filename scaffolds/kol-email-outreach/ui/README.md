...
面向红人营销团队的中文首次建联工作台。系统消费 TikHub 人物画像结果，按项目管理达人、英文邮件母模板、投递通道、人工 Reply-To 邮箱、可选 AI 个性化、人工审批和安全发送。当前 MVP 只完成首封投递，达人回复后由团队在邮箱中人工沟通。
...
- 顺序发送、保存唯一尝试ID和服务商 Message-ID，并在投递结果不明确时全局熔断。
...
TikHub Analyzer CSV
  ...
  → Vinext 中文工作台
       ...
       └─ 127.0.0.1:8878 本地网关
            ├─ AI Key（只写、仅内存）
            ├─ Amazon SES API 凭据（只写、仅内存）
            ├─ Gmail / Outlook OAuth 令牌（只写、仅内存）
            ├─ 自定义 SMTP 应用密码（只写、仅内存）
            ├─ AI 个性化结果缓存
            └─ 分链路限流、顺序发送队列、journal、运行锁、熔断器
```

浏览器不会逐封调用邮件服务商。它只提交一次已审批批次；真正的 AI、Amazon SES API、
Gmail API、Microsoft Graph 或 SMTP 调用均由仅监听本机的网关执行。
...
- 个人邮箱池：Gmail、Outlook 或自定义 SMTP，每个账号单独授权；服务端强制每天最多 100 封、同账号至少间隔 30 秒。
- 企业客户邮箱：客户自己的 Google Workspace、Microsoft 365、品牌 SMTP；服务端强制每天最多 500 封、同账号至少间隔 10 秒。
- 企业 SES：只接受已验证域名且 DKIM 成功的 SES 身份；LOOP 上限每天 5,000 封、至少间隔 2 秒，AWS 实际额度仍优先。

两条链路都共用不可变审批、幂等键、收件人去重、全局停止联系名单和未知投递熔断。系统不保证进入收件箱，也不通过轮换账号绕过服务商限制。
...
- 个人邮箱池：Gmail、Outlook 或自定义 SMTP，每个账号分别授权；
- 企业客户邮箱：客户自己的 Google Workspace、Microsoft 365、品牌 SMTP 或 Amazon SES；
- Amazon SES API：填写 Region、Access Key ID、Secret Access Key、发件邮箱和 Reply-To；AWS 身份至少需要 `ses:GetAccount`、`ses:GetEmailIdentity` 和 `ses:SendEmail`，连接测试要求发件域名身份与 DKIM 已就绪，不会发送测试邮件；
- Gmail / Outlook：填写邮箱、显示名称和每日上限，点击“连接并授权”，在服务商官方页面完成一次 OAuth 授权；
- 自定义 SMTP：另填 SMTP Host、端口、TLS 和应用密码；
- Reply-To 建议填写团队日常处理回复的邮箱。

连接后必须通过“测试连接”。AWS 凭据、OAuth Token 和 SMTP 密码都只存在当前网关进程内存中；
网关重启后需要重新授权或重新输入凭据。OAuth 回调使用一次性 `state` 和 PKCE，授权账号
必须与填写的发件邮箱完全一致。
...
- 每封邮件在调用邮件服务商前写入 `sending` journal 并分配唯一尝试ID；明确成功后保存服务商返回的 Message-ID。
- 任务和逐封结果持久化到 `server-data/send-jobs.json`，该目录不会提交。
- 网关重启不会自动重发未完成邮件，而是保守暂停。
- API/SMTP 超时、进程中断或结果无法可靠落盘时进入 `delivery_unknown`。
- 一旦存在 `delivery_unknown`，全局熔断器阻止所有后续发送，必须人工核对 Sent 文件夹或服务商记录。
- 个人邮箱、普通企业邮箱和 SES 分别执行 100/500/5000 的 LOOP 单日硬上限，以及 30/10/2 秒的同账号最小尝试间隔。
- 英文正文必须保留系统退出语；拒绝、退订、投诉或退信地址可加入全局停止联系名单，后续所有项目生成前都会拦截。
- MVP 默认不主动添加开信像素或点击重写。
...
- AWS 凭据、OAuth Token、SMTP 密码和 AI Key 都是只写、仅内存凭据。
...
1. 连接 Amazon SES、Gmail、Outlook 或企业测试邮箱并点击“测试连接”。
2. 新建仅包含 1–3 个内部收件地址的测试项目。
3. 生成并逐封审批，确认模板报价、链接和签名不被 AI 改写。
4. 正式发送后在“发送结果”核对状态、时间和 Message-ID，并确认回复进入人工邮箱。
5. 重复点击发送，确认不会产生第二封；不要用真实达人验证异常场景。

当前 MVP 不读取客户收件箱、不自动识别回复、不自动 Follow-up。达人回复直接进入 Reply-To 邮箱，由人工接管。
...
测试不会连接真实 API/SMTP，也不会发送邮件。覆盖 AWS/AI/SMTP 凭据不回显、模板保护、画像证据、审批哈希、批次幂等、网关重启恢复、Message-ID 记录和 `delivery_unknown` 熔断。
